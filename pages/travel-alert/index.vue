<template>
  <main class="container mb-64">
    ...
    <div class="w-full h-full md:w-1/3 align-middle relative">
      ...
      <ul v-if="optionsShowed" class="absolute text-gray-700 pt-1 z-10 w-full">
    ...
    >
      <div class="mb-5 flex font-bold rounded-t">
        <div>
          <Flag :country-code="ta.countryCode" />
          {{ ta.countryName }}
          <a :name="ta.countryCode" class="anchor" />
        </div>
        <div class="ml-auto">
          {{ ta.publishedDateFormatted }}
        </div>
      </div>

      <p
        v-for="(message, index) in ta.alertMessages"
        :key="index"
        class="mb-2 text-md"
      >
        {{ message }}
