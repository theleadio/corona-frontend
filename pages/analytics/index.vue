    ...
    <div class="w-full lg:w-5/6 px-2 pt-2 bg-gray-200 relative">
      <div class="pl-2">
        ...
        <p class="text-xs font-bold leading-tight my-2 text-gray-600">
          {{ $t("sources") }}: {{ $t("WHO") }}, CDC, ECDC, NHC of the PRC, JHU
          CSSE, DXY, QQ, {{ $t("international_media") }}
        </p>
      </div>
      ...
      <div class="flex flex-wrap">
        <div class="w-full lg:w-1/2 px-2">
          ...
          <div
            class="w-full rounded shadow-md bg-white p-3 mb-5 block lg:hidden"
          >
            <HintClickCountry />
                  ...
                  <tr v-for="loc in affectedCountryData" :key="loc.country">
                    ...
                    >
                      ...
                      <template v-else-if="loc.countryCode">
                        <nuxt-link
                          :to="`/country/${loc.countryCode.toLowerCase()}`"
                          style="display: block;"
                        >
          ...
          <div class="max-w-full rounded shadow-md bg-white p-3 mb-5">
            <OutbreakTrendChart :data="outbreakTrendData" />
          </div>

          <div class="max-w-full rounded shadow-md bg-white p-3 mb-5">
            <TopCountryWithDailyNewStats
              :data="topCountryWithDailyNewStatsData"
            />
          </div>
        </div>
        ...
        <div class="w-full lg:w-1/2 px-2">
          <div class="max-w-full rounded shadow-md bg-white p-3 mb-5">
            <AffectedCountry :data="affectedCountryData" />
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    loadStats() {
      this.$api.stats.getGlobalStats().then(data => {
    ...
    loadOutbreakTrend() {
      const limit = 100
      this.$api.analytics.fetchTopTrendingCases(limit).then(data => {
    ...
    async loadAffectedCountry() {
      ...
      this.affectedCountryData = items.filter(
        a => a.countryCode && a.totalConfirmed
      )
    }
  },
  head() {
    ...
    return {
      ...
      meta: [
        ...
        },
        {
          hid: "og-image",
          property: "og:image",
          content: process.env.API_BASE_URL + "/share/globalStatsToday"
        },
        {
          hid: "twitter-image",
