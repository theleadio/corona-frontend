<template>
  <div>
    <Card :title="$t('screening_centres')" />
    <dropdown
      v-model="region"
    ...
    <div v-if="filteredLocations.length > 0">
      ...
      >
        <p class="leading-normal font-bold capitalize">
          <i class="fas fa-map-marker-alt" /> {{ location.name }}
        </p>
        <p class="text-xs">
          {{ location.address }}
        </p>
        <p class="text-right font-semibold">
          <a :href="`tel:${location.telNo}`"
            ><i class="fas fa-phone" /> {{ location.telNo }}</a
          >
        </p>
      </div>
    </div>
    <div
      v-else-if="
        country === 'global' ||
          locations.some(
            loc => loc.country.toUpperCase() === country.toUpperCase()
          )
      "
    ...
    >
      ...
      <p class="font-bold text-center capitalize">
        {{ $t("select_region") }}
      </p>
    </div>
    <div
    ...
    >
      ...
      <p class="text-center">
        {{
          $t(
            "We do not have any screening centre location data for the selected country or region yet!"
          )
        }}
      </p>
    ...
    >
      ...
      <p class="font-bold text-center text-primary text-lg">
        ...
        >
          <i class="fab fa-telegram" /> {{ $t("join_telegram") }}
        </a>
      </p>
    </div>
  </div>
    ...
    filteredRegions: function() {
      if (this.country === "" || this.country === "global") {
        ...
        return this.regions.filter(
          region => region.country.toUpperCase() === this.country.toUpperCase()
        )
      }
    },
