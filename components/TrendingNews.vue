...
export default {
	...
	methods: {
		loadData() {
			...
			this.ajax = this.$api.news.getTrendingNews({ limit, offset, country: this.country.name, language: this.selectedLanguages.join(',') })
				.then(data => {
					this.articles = data.items && data.items.map(news => {
