<template>
  <div
    class="max-w-full rounded border border-gray-400 bg-white p-3 h-full relative"
  >
    <div class="flex flex-wrap justify-between items-center">
      ...
      <Share :url="shareUrl + '?referrer=today'" />
    </div>

    <div
      class="flex flex-row lg:flex-row pt-3 justify-around sm:justify-around md:justify-around lg:justify-around"
    >
      <div class="px-2 text-center">
        <p class="text-2xl font-bold text-red-600">
          {{ info.confirmed | formatNumber }}
        </p>
        <p class="text-l font-bold text-gray-600">
          {{ $t("confirmed") }}
        </p>
        <p class="text-xs font-bold text-red-600">
          {{
            $t("increase_new_cases", {
              number: $options.filters.formatNumber(info.diffConfirmed)
            })
          }}
        </p>
      </div>
        ...
        <p class="text-xs font-bold text-gray-600">
          {{
            $t("increase_new_deaths", {
              number: $options.filters.formatNumber(info.diffDeaths)
            })
          }}
        </p>
      </div>
