<template>
  <div class="flex flex-col items-center search" v-on-clickaway="onClickAwaySearch">
    ...
    <div v-else-if="state === 'done' && !results.length" class="flex flex-col w-full overflow-auto bg-white text-black shadow-md list p-4">
...
export default {
  ...
  methods: {
    performSearch: function () {
      ...
      this.$api.news.searchNews(this.term).then(value => {
        this.results = value;
        this.state = "done";
