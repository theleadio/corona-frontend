...
export default {
  ...
  watch: {
    data(val) {
      const categories = val.map(i =>
        moment(i.lastUpdated, "YYYY-MM-DD").format("YYYY-MM-DD")
      )
      const series = [
        {
      ...
      val
        .map(i => i)
        .forEach(item => {
          const categoryIdx = categories.indexOf(
            moment(item.lastUpdated, "YYYY-MM-DD").format("YYYY-MM-DD")
          )
