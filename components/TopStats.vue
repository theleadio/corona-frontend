<template>
  <div>
    <Card v-if="showTitle" :title="$t('countries_affected')" />
    <div
      v-if="showSource"
      class="my-2 font-bold text-xs text-gray-500 leading-tight"
    >
      {{ $t("sources") }}: {{ $t("WHO") }}, CDC, ECDC, NHC of the PRC, JHU CSSE,
      DXY, QQ, {{ $t("international_media") }}
    </div>
    ...
    <table class="table-auto w-full">
      <thead class="text-xs leading-tight border-b-2">
        <tr>
          ...
          <th
            class="border px-1 py-2 pointer"
            @click="headerClick('confirmed')"
          >
            {{ $t("confirmed")
            }}<span
              v-if="sortField === 'confirmed'"
              :class="{
                'up-arrow': !sortInDescendingOrder,
                'down-arrow': sortInDescendingOrder
              }"
              class="sorter-header"
            />
          </th>
          <th
            class="border px-1 py-2 pointer"
            @click="headerClick('recovered')"
          >
            {{ $t("recovered")
            }}<span
              v-if="sortField === 'recovered'"
              :class="{
                'up-arrow': !sortInDescendingOrder,
                'down-arrow': sortInDescendingOrder
              }"
      ...
      <tbody class="font-bold">
        ...
        <template v-else>
          <tr v-for="item in countryStats" :key="item.countryCode">
            <td
              class="bg-gray-200 text-xs border hover:bg-primary hover:text-white px-2 py-2 "
            >
              <template v-if="item.countryCode === 'OT'">
                <div :style="usedInSharePage ? 'margin-left: 35%;' : ''">
                  {{ item.country }}<a href="#notes-on-others">*</a>
                </div>
              </template>
              <template v-else-if="item.countryCode">
                ...
                >
                  <Flag :country-code="item.countryCode" />
                  {{ item.country }}
                </nuxt-link>
              </template>
            </td>
    ...
    <div
      v-if="showFooter"
      class="my-2 font-bold text-xs text-gray-600 leading-tight"
    >
